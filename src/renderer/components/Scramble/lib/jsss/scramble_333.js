...
scramblers['333'] = (function () {
  ...
  function $invCubieCube (obj) {
    ...
    for (edge = 0; edge < 12; ++edge) { obj.temps.eo[edge] = obj.eo[obj.temps.ep[edge]] }
  ...
  let _ = CubieCube_3.prototype = CubieCube_2.prototype = CubieCube_0.prototype = CubieCube.prototype
  _.temps = null
  ...
  function toFaceCube (cc) {
    ...
    for (c = 0; c < 8; ++c) {
      j = cc.cp[c]
      ori = cc.co[c]
      for (n = 0; n < 3; ++n) { f[cornerFacelet[c][(n + ori) % 3]] = ts[cornerColor[j][n]] }
    }
    for (e = 0; e < 12; ++e) {
      j = cc.ep[e]
      ori = cc.eo[e]
      for (n = 0; n < 2; ++n) { f[edgeFacelet[e][(n + ori) % 2]] = ts[edgeColor[j][n]] }
    }
...
const scrambler = scramblers['333']
